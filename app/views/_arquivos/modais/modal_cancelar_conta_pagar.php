<div class="modal fade" id="modal_cancelar_conta_pagar">
        ...
        <div class="col-md-12">
            ...
            <div class="box-body">
                ...
                <form method="POST" id="form-cancelar-conta" name="form-cancelar-conta">
                    ...
                    <input type="hidden" class="form-control" id="id_conta_pagar_compensar" name="id_conta_pagar_compensar">
...
$(function(){
    ...
    $("#modal_cancelar_conta_pagar .erros").html(""); 
    $("#form-cancelar-conta").submit(function(event){
    event.preventDefault();
    var formDados = new FormData($(this)[0]);
    var erros = validarForm("#form-cancelar-conta");
    if(erros.length > 0){
        $("#modal_cancelar_conta_pagar .erros").html(erros).css({'color':'red','font-weight':'bold','font-size':'14pt'}); 
        alert("aqui");
        return false;
    }else{
        
        $.post(
                <?=DOMINIO?>+"contaspagar/cancelarconta",
                   $("#form-cancelar-conta").serialize(),
                   function (response, status){
                       
                   $("#modal_cancelar_conta_pagar .erros").html(response).css({'color':'green','font-weight':'bold','font-size':'14pt'}); 
...
var erros = "";
    ...
    if($(form+" #id_conta_pagar_compensar").val().length <= 0){
